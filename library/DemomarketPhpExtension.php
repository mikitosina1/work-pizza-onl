<?php
	...
	class DemomarketPhpExtension extends ViewPhpExtension {
		...
		public function initializeCommonVariables($variables) {
            ...
            //мои переменные Никита
            $templateEngine->setCommonVar('mydomain', $variables['domain-url']);
            $templateEngine->setCommonVar('temp', '/templates/demomarket/');
		}
        /** @Достаёт название элемента выпадающего меню (элемента справочника) */
        public function get_sprav($id_sp) {
            $sprav = '';
            $sprav = umiObjectsCollection::getInstance()->getObject($id_sp);
            return  $sprav != '' ? $value = $sprav->getName() : '';
        }
        
        /** Достаёт количество эл-тов из корзины */
		public function count_k($id_toppings,$output_order) {
            $count = 0;
            foreach($output_order['items'] as $key => $valuez){
                $topping_to_show = $valuez['element_id'];
                $count = $id_toppings == $topping_to_show ? $valuez['amount'] : $count;
                
                //print_r($output_order['items'].'<br>');
		...
		public function getMetaDescription(array $variables) {
			if (!class_exists('SeoPhpExtension', true)) {
				return '';
			}
